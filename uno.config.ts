...
export default defineConfig({
  ...
  rules: [
    ...
    [/^slide-in-from-t(op)?-?(.+)?$/, ([,, d]) => ({ '--un-enter-translate-y': `-${handleMatchRem(d)}` })],
    [/^slide-in-from-b(ottom)?-?(.+)?$/, ([,, d]) => ({ '--un-enter-translate-y': handleMatchRem(d) })],
    [/^slide-in-from-l(eft)?-?(.+)?$/, ([,, d]) => ({ '--un-enter-translate-x': `-${handleMatchRem(d)}` })],
    [/^slide-in-from-r(ight)?-?(.+)?$/, ([,, d]) => ({ '--un-enter-translate-x': handleMatchRem(d) })],
    [/^slide-out-to-t(op)?-?(.+)?$/, ([,, d]) => ({ '--un-exit-translate-y': `-${handleMatchRem(d)}` })],
    [/^slide-out-to-b(ottom)?-?(.+)?$/, ([,, d]) => ({ '--un-exit-translate-y': handleMatchRem(d) })],
    [/^slide-out-to-l(eft)?-?(.+)?$/, ([,, d]) => ({ '--un-exit-translate-x': `-${handleMatchRem(d)}` })],
    [/^slide-out-to-r(ight)?-?(.+)?$/, ([,, d]) => ({ '--un-exit-translate-x': handleMatchRem(d) })],
  ],
  shortcuts: [
    ['btn', 'px-4 py-1 rounded inline-block bg-accent9 text-accent-fg cursor-pointer hover:bg-accent10 disabled:cursor-default disabled:bg-gray9 disabled:opacity-50'],
